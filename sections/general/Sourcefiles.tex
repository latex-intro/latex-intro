...
\begin{frame}[fragile]
    ...
    Latex kann mit Paketen erweitert werden. Dies ist für viele Dinge sehr wichtig, da die Standardfunktionen zum Teil nicht ausreichen.\pause
    \begin{block}{Paket mit Optionen laden}
        \cmd{usepackage}\oarg{Optionen}\marg{Paketname}
    \end{block}
    \pause
    \bigskip
    \begin{block}{Quelldatei ist UTF-8, immer nutzen}
		\verb+\usepackage[utf8]{inputenc}+
    \end{block}
    \pause
    \begin{block}{Sprache auf Deutsch umstellen}
...
\begin{frame}[fragile]
    ...
    \framesubtitle{Das \pkg{babel}-Paket}
    
    Sehr wichtiges Paket zur Lokalisierung:\pause
    \begin{itemize}
        \item Deutsche Überschriften für z.B. Verzeichnisse\pause
        \item Deutsche Silbentrennung\pause
        \item Deutsche Anführungszeichen mit \verb+"`Hallo"'+: "`Hallo"'\pause
    \end{itemize}
    \bigskip
